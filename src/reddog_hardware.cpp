...
void Custom::LowCmdMessageHandler(const void *message)
{
    ...
    // 如果你要印全部的 motorCmd，可用 for 迴圈：
    for (int i = 0; i < 12; ++i) {
        std::cout << "MotorCmd[" << low_cmd.motor_cmd()[i].mode() << "]"
                  << " q: " <<      low_cmd.motor_cmd()[i].q()
                  << " dq: " <<     low_cmd.motor_cmd()[i].dq()
                  << " tau: " <<    low_cmd.motor_cmd()[i].tau()
                  << " Kp: " <<     low_cmd.motor_cmd()[i].kp()
                  << " Kd: " <<     low_cmd.motor_cmd()[i].kd()
                  << std::endl;
    }
}
